...
void main() {
  ...
  testWidgets('edge swipe closes a chat with composer open', (tester) async {
    tester.view.physicalSize = const Size(1080, 2400);
    tester.view.devicePixelRatio = 3;
    addTearDown(tester.view.resetPhysicalSize);
    addTearDown(tester.view.resetDevicePixelRatio);

    final navigator = await _openChat(tester);
    expect(navigator.canPop(), isTrue);

    await tester.tap(find.byTooltip('Attachments'));
    await tester.pumpAndSettle();
  ...
  testWidgets('system back closes composer before leaving chat', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(1080, 2400);
    tester.view.devicePixelRatio = 3;
...
Future<NavigatorState> _openChat(WidgetTester tester) async {
  final repository = MockChatRepository();
  await repository.connect();
  addTearDown(repository.disconnect);
  final chat = ChatSummary(
